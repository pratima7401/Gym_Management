...
function MemberRegistration() {
  const [formData, setFormData] = useState({
    ...
    joinDate: "",
    gender: "",
    membershipPlans: [], // Updated to handle multiple selected plans
  });
  ...
  const fetchPlans = async () => {
    try {
      const response = await axios.get(
        "http://localhost/api.php?action=getMembershipPlans"
      );
  ...
  const handleCheckboxChange = (e) => {
    ...
    setFormData((prevData) => {
      const updatedPlans = checked
        ? [...prevData.membershipPlans, value]
        : prevData.membershipPlans.filter((plan) => plan !== value);
      return { ...prevData, membershipPlans: updatedPlans };
    });
  };
